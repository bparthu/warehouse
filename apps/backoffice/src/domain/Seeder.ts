import { Inventory, Product, ProductInventory, Sequelize, getConnection } from "@warehouse/core";
...
const seedProductInventory = async (seederPath: string) => {
  ...
  const rawData = await getRawData<ProductSeed>(seederPath, "products")
  // map and prepare data to write to db
  const records = rawData.products.map((productItem) => productItem.contain_articles.map(({ art_id: inventoryId, amount_of: amountOf }) => ({
    inventoryId,
