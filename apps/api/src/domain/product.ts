...
const isProductAvailable = (products: Product) => (Math.min(...products.inventories.map((inventory) => (inventory.stock - inventory.getDataValue("ProductInventory").getDataValue("amountOf")))) >= 0)
...
const getProducts = async () => {
  ...
  const productCount = products.map((product) => {
    ...
    const stock = Math.min(...product.inventories.map((inventory) => {
      return Math.floor(inventory.getDataValue("stock") / inventory.getDataValue("ProductInventory").getDataValue("amountOf"))
    }))
...
const deleteProduct = async (productId: string) => {
  ...
  const prepareInventoriesToUpdate = product.inventories.map((inventory) => ({
    ...
    numStocksToReduce: inventory.getDataValue("ProductInventory").getDataValue("amountOf") as number
  }))
  
  await Promise.all(prepareInventoriesToUpdate.map((lookupData) => {
